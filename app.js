...
const SCREENS = ['select-screen','grammar-screen','app-screen','exam-screen','quiz-screen','result-screen','fc-setup-screen','fc-play-screen','fc-end-screen','pt-setup-screen','pt-play-screen','pt-result-screen','py-setup-screen','py-play-screen','py-result-screen','vt-setup-screen','vt-play-screen','vt-result-screen','hw-setup-screen','hw-play-screen','hw-result-screen'];
let currentProfile = '';
...
function restoreAppState(){
  try{
    ...
    if(['hw-setup-screen','hw-play-screen','hw-result-screen'].includes(state.screen)){
      showHwSetup();
      return true;
    }
    showOnly('select-screen');
...
/* ===================== LUYỆN VIẾT (HanziWriter) ===================== */
var hwCfg = {pool:'all', order:'seq', count:10};
var hwShowOutline = true;
var hwShowHint = true;
var hwCards = [], hwIdx = 0, hwResults = [];
var hwCharList = []; // characters of current word
var hwCharIdx = 0;
var hwWordMistakes = 0;
var hwWordDone = false;
var hwWriter = null;
var hwQuizActive = false;

function showHwSetup(){
  var t = VOCAB.length, s = starred.size;
  document.getElementById('hw-total').textContent = t;
  document.getElementById('hw-starred').textContent = s;
  document.getElementById('hw-unstarred').textContent = Math.max(0, t - s);
  hwCfg = {pool:'all', order:'seq', count:10};
  hwShowOutline = true;
  hwShowHint = true;
  var outlineT = document.getElementById('hw-outline-toggle');
  var hintT = document.getElementById('hw-hint-toggle');
  if(outlineT) outlineT.className = 'toggle-switch on';
  if(hintT) hintT.className = 'toggle-switch on';
  document.querySelectorAll('#hw-setup-screen .opt-btn').forEach(function(b,i){
    // activate first of each option-group
  });
  document.querySelectorAll('#hw-setup-screen .setup-section').forEach(function(sec){
    var btns = sec.querySelectorAll('.opt-btn');
    btns.forEach(function(b,i){ b.classList.toggle('active', i===0); });
  });
  var custom = document.getElementById('hw-custom-count');
  if(custom) custom.value = '';
  showOnly('hw-setup-screen');
}

function setHwOpt(k, v, btn){
  hwCfg[k] = v;
  if(k==='count' && v===0) hwCfg.count = VOCAB.length;
  if(btn){
    btn.parentElement.querySelectorAll('.opt-btn').forEach(function(b){ b.classList.remove('active'); });
    btn.classList.add('active');
    if(k==='count'){
      var c = document.getElementById('hw-custom-count');
      if(c) c.value = '';
    }
  }
}

function toggleHwOutline(){
  hwShowOutline = !hwShowOutline;
  document.getElementById('hw-outline-toggle').className = 'toggle-switch' + (hwShowOutline ? ' on' : '');
}

function toggleHwHint(){
  hwShowHint = !hwShowHint;
  document.getElementById('hw-hint-toggle').className = 'toggle-switch' + (hwShowHint ? ' on' : '');
}

function hwExtractChars(hanzi){
  var chars = [];
  var s = String(hanzi || '');
  for(var i = 0; i < s.length; i++){
    var ch = s.charAt(i);
    // CJK Unified Ideographs + common extensions / compatibility
    if(/[\u3400-\u9FFF\uF900-\uFAFF]/.test(ch)) chars.push(ch);
  }
  return chars;
}
...
function startHw(){
  if(typeof HanziWriter === 'undefined'){
    alert('Không tải được thư viện HanziWriter. Kiểm tra kết nối mạng và thử lại.');
    return;
  }
  var pool = [];
  for(var i = 0; i < VOCAB.length; i++){
    if(hwCfg.pool === 'starred' && !starred.has(i)) continue;
    if(hwCfg.pool === 'unstarred' && starred.has(i)) continue;
    // only words that have at least one Chinese character
    if(!hwExtractChars(VOCAB[i].hanzi).length) continue;
    pool.push(i);
  }
  if(!pool.length){
    alert('Không có từ nào phù hợp!');
  ...
  var n = hwCfg.count || pool.length;
  hwCards = pool.slice(0, Math.min(n, pool.length));
  hwIdx = 0;
  hwResults = [];
  hwWordMistakes = 0;
  hwWordDone = false;
  showOnly('hw-play-screen');
  hwRenderWord();
}

function hwDestroyWriter(){
  hwQuizActive = false;
  if(hwWriter){
    try{ if(hwWriter.cancelQuiz) hwWriter.cancelQuiz(); }catch(e){}
    hwWriter = null;
  }
...
function hwRenderWord(){
  if(hwIdx >= hwCards.length){
    hwShowResult();
    return;
  }
  var ci = hwCards[hwIdx];
  var w = VOCAB[ci];
  var tot = hwCards.length;
  hwCharList = hwExtractChars(w.hanzi);
  hwCharIdx = 0;
  hwWordMistakes = 0;
  hwWordDone = false;

  document.getElementById('hw-prog-text').textContent = (hwIdx + 1) + ' / ' + tot;
  document.getElementById('hw-prog-bar').style.width = ((hwIdx + 1) / tot * 100) + '%';
  document.getElementById('hw-word-hanzi').textContent = w.hanzi;
  document.getElementById('hw-word-pinyin').textContent = w.pinyin || '';
  document.getElementById('hw-word-meaning').textContent = w.meaning || '';
  document.getElementById('hw-next-btn').classList.add('hidden');
  document.getElementById('hw-status').textContent = 'Vẽ từng nét theo đúng thứ tự';
  document.getElementById('hw-status').className = 'hw-status';

  // Character tabs for multi-char words
  var tabs = document.getElementById('hw-char-tabs');
  if(hwCharList.length <= 1){
    tabs.innerHTML = '';
    tabs.style.display = 'none';
  } else {
    tabs.style.display = 'flex';
    tabs.innerHTML = hwCharList.map(function(ch, i){
      return '<button type="button" class="hw-char-tab' + (i === 0 ? ' active' : '') + '" data-i="' + i + '" onclick="hwJumpChar(' + i + ')">' + esc(ch) + '</button>';
    }).join('');
  }
...
function hwUpdateTabs(){
  var tabs = document.getElementById('hw-char-tabs');
  if(!tabs || hwCharList.length <= 1) return;
  var nodes = tabs.querySelectorAll('.hw-char-tab');
  nodes.forEach(function(el, i){
    el.classList.toggle('active', i === hwCharIdx);
    el.classList.toggle('done', i < hwCharIdx);
  });
}

function hwJumpChar(i){
  if(hwWordDone) return;
  if(i < 0 || i >= hwCharList.length) return;
  // only allow jumping to current or previous chars during practice
  if(i > hwCharIdx) return;
  hwCharIdx = i;
  hwUpdateTabs();
  hwLoadChar();
}
...
function hwGetSize(){
  var wrap = document.querySelector('.hw-writer-wrap');
...
function hwLoadChar(){
  hwDestroyWriter();
  if(!hwCharList.length){
    document.getElementById('hw-status').textContent = 'Không có chữ Hán hợp lệ — bỏ qua từ này';
    document.getElementById('hw-status').className = 'hw-status hw-status-err';
    document.getElementById('hw-next-btn').classList.remove('hidden');
    return;
  }
  var ch = hwCharList[hwCharIdx];
  hwUpdateTabs();
  document.getElementById('hw-status').textContent =
    (hwCharList.length > 1
      ? ('Chữ ' + (hwCharIdx + 1) + '/' + hwCharList.length + ' · ')
      : '') + 'Vẽ chữ 「' + ch + '」';
  document.getElementById('hw-status').className = 'hw-status';
  ...
  try {
    ...
    hwWriter = HanziWriter.create('hw-target', ch, {
      width: size,
      height: size,
      padding: 20,
      showOutline: hwShowOutline,
      showCharacter: false,
      strokeColor: '#1a0a00',
      radicalColor: '#c0392b',
      outlineColor: '#ddd0c0',
      ...
      onLoadCharDataError: function(){
        loadFailed = true;
        document.getElementById('hw-status').textContent = 'Không tải được dữ liệu nét cho 「' + ch + '」';
        document.getElementById('hw-status').className = 'hw-status hw-status-err';
        setTimeout(function(){ hwOnCharComplete(true); }, 700);
      }
    });
    // quiz() tự chờ char data load xong
    if(!loadFailed) hwStartQuiz();
  } catch(err){
    console.warn('HanziWriter create error', err);
    document.getElementById('hw-status').textContent = 'Không tải được dữ liệu nét cho 「' + ch + '」';
    document.getElementById('hw-status').className = 'hw-status hw-status-err';
    setTimeout(function(){ hwOnCharComplete(true); }, 600);
  }
}

function hwStartQuiz(){
  if(!hwWriter) return;
  hwQuizActive = true;
  try {
    hwWriter.quiz({
      showHintAfterMisses: hwShowHint ? 2 : false,
      showCharacter: false,
      onCorrectStroke: function(data){
        var total = data.strokeNum + 1;
        var drawn = data.drawnPath ? total : (data.strokeNum + 1);
        // data has strokesRemaining, strokeNum, mistakesOnStroke, totalMistakes
        var remaining = (typeof data.strokesRemaining === 'number') ? data.strokesRemaining : null;
        if(remaining !== null){
          document.getElementById('hw-status').textContent =
            '✓ Nét ' + (data.strokeNum + 1) + ' đúng' +
            (remaining > 0 ? ' · còn ' + remaining + ' nét' : '');
          document.getElementById('hw-status').className = 'hw-status hw-status-ok';
        }
      },
      onMistake: function(data){
        hwWordMistakes++;
        var miss = data.mistakesOnStroke || 1;
        document.getElementById('hw-status').textContent =
          '✗ Sai nét (lần ' + miss + ') · thử lại';
        document.getElementById('hw-status').className = 'hw-status hw-status-err';
      },
      onComplete: function(summary){
        hwQuizActive = false;
        if(summary && typeof summary.totalMistakes === 'number'){
          // mistakes already counted per stroke; ensure total is at least summary
          // (we count live, summary is authoritative for this char)
        }
        document.getElementById('hw-status').textContent = '🎉 Hoàn thành chữ 「' + hwCharList[hwCharIdx] + '」!';
        document.getElementById('hw-status').className = 'hw-status hw-status-ok';
        setTimeout(function(){ hwOnCharComplete(false); }, 700);
      }
    });
  } catch(e){
    console.warn('quiz start error', e);
    document.getElementById('hw-status').textContent = 'Không bắt đầu được chế độ viết — thử xem nét hoặc bỏ qua';
    document.getElementById('hw-status').className = 'hw-status hw-status-err';
  }
}

function hwOnCharComplete(failedLoad){
  if(hwWordDone) return;
  if(failedLoad){
    // treat missing data as skip of that char, continue
  }
  if(hwCharIdx < hwCharList.length - 1){
    hwCharIdx++;
    hwLoadChar();
    return;
  }
  ...
  document.getElementById('hw-status').textContent =
    '✅ Xong từ 「' + w.hanzi + '」' +
    (hwWordMistakes ? (' · ' + hwWordMistakes + ' lần vẽ sai') : ' · hoàn hảo!');
  document.getElementById('hw-status').className = 'hw-status hw-status-ok';
  document.getElementById('hw-next-btn').classList.remove('hidden');
  // auto-advance after short delay if not last
  if(hwIdx < hwCards.length - 1){
    setTimeout(function(){
      if(hwWordDone && document.getElementById('hw-play-screen').classList.contains('active')){
        // only auto if user hasn't already clicked
        if(!document.getElementById('hw-next-btn').classList.contains('hidden')){
          // leave button for user control - don't force auto
        }
      }
    }, 900);
  } else {
    document.getElementById('hw-next-btn').textContent = 'Xem kết quả →';
  }
}
...
function hwAnimate(){
  if(!hwWriter) return;
  try{
    if(hwQuizActive && hwWriter.cancelQuiz) hwWriter.cancelQuiz();
    hwQuizActive = false;
    hwWriter.hideCharacter();
    if(hwShowOutline) hwWriter.showOutline();
    hwWriter.animateCharacter({
      onComplete: function(){
        document.getElementById('hw-status').textContent = 'Đã xem xong thứ tự nét — bấm 「Viết lại」 để luyện';
        document.getElementById('hw-status').className = 'hw-status';
      }
    });
    document.getElementById('hw-status').textContent = '🎬 Đang chiếu thứ tự nét...';
    document.getElementById('hw-status').className = 'hw-status';
...
function hwShowHint(){
  if(!hwWriter) return;
  try{
    // highlight next stroke if quiz supports it
    if(hwWriter.highlightStroke){
      // During quiz, use quiz's built-in; otherwise animate first stroke
    }
    if(typeof hwWriter._quiz !== 'undefined' && hwWriter._quiz){
      // public API: give a free hint by canceling and restarting is heavy;
      // use highlightStroke via internal if available, else re-animate
    }
    // Reliable public approach: animate character briefly as hint
    if(hwQuizActive && hwWriter.cancelQuiz) hwWriter.cancelQuiz();
    hwQuizActive = false;
    hwWriter.animateCharacter({
      onComplete: function(){
    ...
    document.getElementById('hw-status').textContent = '💡 Gợi ý: đang chiếu nét...';
    document.getElementById('hw-status').className = 'hw-status';
  }catch(e){
    console.warn(e);
  }
...
function hwRestartChar(){
  if(hwWordDone) return;
  hwLoadChar();
  document.getElementById('hw-status').textContent = '↺ Viết lại từ đầu';
  document.getElementById('hw-status').className = 'hw-status';
}

function hwSkip(){
  if(hwIdx >= hwCards.length) return;
  var ci = hwCards[hwIdx];
  var w = VOCAB[ci];
  hwDestroyWriter();
  hwResults.push({
  ...
  hwWordDone = false;
  document.getElementById('hw-next-btn').classList.add('hidden');
  document.getElementById('hw-next-btn').textContent = 'Từ tiếp theo →';
  hwRenderWord();
}

function hwNextWord(){
  if(!hwWordDone){
    // if user presses next without completing, treat as skip remaining
    hwSkip();
    return;
  }
  hwIdx++;
...
function hwShowResult(){
  hwDestroyWriter();
  var total = hwResults.length;
  var correctN = hwResults.filter(function(r){ return r.correct; }).length;
  var skippedN = hwResults.filter(function(r){ return r.skipped; }).length;
  var mistakesN = hwResults.reduce(function(s, r){ return s + (r.mistakes || 0); }, 0);
  var pct = total ? Math.round(correctN / total * 100) : 0;

  document.getElementById('hw-res-score').textContent = correctN + '/' + total;
  document.getElementById('hw-res-correct').textContent = correctN;
  document.getElementById('hw-res-mistakes').textContent = mistakesN;
  document.getElementById('hw-res-skipped').textContent = skippedN;
  document.getElementById('hw-res-sub').textContent =
    pct >= 80 ? '🌟 Xuất sắc! Nét chữ rất chắc!' :
    pct >= 60 ? '👍 Tốt! Tiếp tục luyện tập!' :
    pct >= 40 ? '💪 Ổn! Cần ôn luyện thêm' :
    '📚 Cần rèn luyện nhiều hơn!';

  var list = document.getElementById('hw-res-list');
  list.innerHTML = hwResults.map(function(r){
    var icon = r.skipped ? '⏭' : (r.correct ? '✅' : '❌');
    var cls = r.skipped ? 'wrong' : (r.correct ? 'correct' : 'wrong');
    return '<div class="result-item ' + cls + '">' +
      '<div class="pt-ri-icon">' + icon + '</div>' +
      '<div class="pt-ri-content">' +
        '<div class="pt-ri-hanzi">' + esc(r.hanzi) + '</div>' +
        '<div class="pt-ri-pinyin">' + esc(r.pinyin) + ' · ' + esc(r.meaning) + '</div>' +
        '<div class="pt-ri-recognized">' +
          (r.skipped ? 'Đã bỏ qua' : ('Hoàn thành · ' + (r.mistakes || 0) + ' lần vẽ sai')) +
        '</div>' +
      '</div>' +
    '</div>';
  }).join('');

  showOnly('hw-result-screen');
}

function hwSaveCorrectToStar(){
  var count = 0;
  hwResults.forEach(function(r){
    if(r.correct && !starred.has(r.idx)){
      starred.add(r.idx);
      count++;
    }
  ...
  saveStars();
  updateStarCount();
  var btn = document.querySelector('#hw-star-save .btn-save-star');
  if(btn){
    btn.textContent = '✅ Đã lưu ' + count + ' từ';
    btn.disabled = true;
...
function hwQuitTest(){
  if(hwIdx === 0 && hwResults.length === 0 && !hwWordDone){
    hwDestroyWriter();
    backToApp();
    return;
  }
  var done = hwResults.length;
  var total = hwCards.length;
  var correctN = hwResults.filter(function(r){ return r.correct; }).length;
  document.getElementById('hw-quit-summary').textContent =
    'Bạn đã luyện ' + done + '/' + total + ' từ, hoàn thành ' + correctN + ' từ.';
  document.getElementById('hw-quit-modal').classList.add('show');
}

function closeHwQuit(){
  document.getElementById('hw-quit-modal').classList.remove('show');
}

function hwQuitSave(){
  hwResults.forEach(function(r){
    if(r.correct) starred.add(r.idx);
  });
  saveStars();
  updateStarCount();
  closeHwQuit();
